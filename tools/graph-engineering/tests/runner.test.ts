...
describe("graph runner", () => {
  ...
  it("fails fast instead of corrupting journal.ndjson when another runner advanced it", async () => {
    ...
    try {
      ...
      const rejected = await b.submit(
        signal("MODEL_DRAFTED", "human", "modeler", { modelHash: "model-b" }, ["forged"]),
      );
      expect(rejected.accepted).toBe(false);

      await expect(
        a.submit(signal("MODEL_CONTRACT_VALID", "tool", "model-contract-validator", {}, ["ok"])),
      ).rejects.toThrow(/concurrent graph runner detected/);
