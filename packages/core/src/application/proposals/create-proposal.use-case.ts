...
export class CreateProposalUseCase {
  ...
  public async execute(command: CreateProposalCommand): Promise<CreateProposalResult> {
    return commitMutation<CreateProposalResult>(this.dependencies.repository, async () => {
      const state = this.dependencies.repository.get();
      if (!state.initialized) {
        return { persist: false, value: { ok: false, error: "DAO not initialized. Run dao_setup first." } };
      }
      ...
      if (missingDependency !== undefined) {
        return {
          persist: false,
          value: { ok: false, error: `Unknown proposal dependency #${missingDependency}.` },
        };
      }
      ...
      if (command.dependsOn && command.dependsOn.length > 0) {
        ...
        if (resolution.error) {
          return { persist: false, value: { ok: false, error: `Invalid dependency graph: ${resolution.error}` } };
        }
      }
      ...
      return { persist: true, value: { ok: true, proposal } };
    });
  }
}
