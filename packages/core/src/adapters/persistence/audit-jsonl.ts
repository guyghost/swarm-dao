...
/** Parse JSONL content tolerantly: empty, torn or corrupt lines are skipped.
 *  Duplicates are NOT removed here — dedupe by id happens in the merge step
 *  (`mergeAuditEntries`) so callers can also fold legacy inline entries in. */
export function parseAuditJsonl(raw: string): AuditEntry[] {
  ...
  for (const line of raw.split("\n")) {
    ...
    try {
      ...
      // Torn tail (crash mid-append) or damaged line: skip, never fatal.
    }
  }
  return entries;
}
