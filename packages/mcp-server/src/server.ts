import type { ProposalType } from "@guyghost/swarm-dao-core";
import {
  buildDaoHelpMessage,
...
const proposalTypeSchema = z.enum(PROPOSAL_TYPES as [string, ...string[]]);

function textResult(text: string) {
  return { content: [{ type: "text" as const, text }] };
}

function createToolContext(workDir: string): DaoToolContext {
  return {
...
export function createSwarmDaoMcpServer(workDir = resolveDaoRoot()): Server {
  ...
  const controlTool: ControlToolName = "dao_control";
        ...
        case "dao_propose":
          return textResult(
            await handleDaoPropose({
              ...
              type: proposalTypeSchema.parse(args.type) as ProposalType,
        ...
        case "dao_record_outputs":
          return textResult(
            await handleDaoRecordOutputs(
              ...
              (args.outputs as Array<{ agentId: string; content: string; durationMs?: number; error?: string }>) ?? [],
            ),
          );
        case "dao_control":
        ...
        case "dao_propose_amendment":
          return textResult(
            await handleDaoProposeAmendment({
              title: String(args.title),
              description: String(args.description),
              amendmentType: args.amendmentType as never,
